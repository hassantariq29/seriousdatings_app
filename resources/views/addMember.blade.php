...
<div class="middle inner-middle">
  ...
  @endif
    ...
    <p><b>Members in Group: </b>{!! $groups['0'] -> membersToAddCount !!}</p>
    ...
    <h3>Add Members</h3><br />
      ...
      @if($groups['0'] -> groupID != null)
           ...
           {!! Form::open(
              array(
              'url' => 'groups/'.$groups["0"] -> groupID.'/addMember',
                ...
                <input type="hidden"  name="groupID" value= "{!! $groups['0'] -> groupID !!}">
                   @if($groups['0'] -> membersToAddCount > 0 ) 
                          <div class="cell2">
                                ...
                                <label for="check-two"><span></span>Check ALL</label>
                          </div>
                  @endif
                @foreach($groups['0'] -> membersToAdd as $group)
                    
                    @if($group -> alreadyMember != 1)
                        ...
                        <div class="grup_member">
                        <div>
                            <img src="{!! url() !!}/images/users/{!! $group -> username !!}/{!! $group -> photo !!}" class="img-responsive" alt="group member image"/>
                        </div>
                        <div class="member_name">
                          <div class="cell">
                            <input type="checkbox" id="membersCheckBox" name="members[]" value= "{!! $group -> friend_id !!}">
                            <label for="check-two"><span></span></label>
                          </div>
